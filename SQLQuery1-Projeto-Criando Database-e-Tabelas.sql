...
--Cadastros complementares por área
--Criando tabelas sem dependências/dependentes
...
--Criação de Tabelas Apontamentos de Produção
...
CREATE TABLE APONTAMENTOS(
	...
	--Login, Será criado após criação da tabela usuários
...
CREATE TABLE ESTOQUE_MOV(
	...
	REFERENCES MATERIAL(COD_EMPRESA, COD_MAT)
	--CAMPO LOGIN, TABELA, ESTOQUE_MOV, CRIAÇÃO APÓS A TABELA USUÁRIO
)

--Criação Tabela PED_COMPRAS
--DROP TABLE PED_COMPRAS
CREATE TABLE PED_COMPRAS(
	COD_EMPRESA INT NOT NULL,
	NUM_PEDIDO INT NOT NULL,
	ID_FOR INT NOT NULL,
	COD_PAGTO INT NOT NULL, --Alterar COD_PAGTO tabela PED_COMPRAS para FOREIGN KEY após Tabela COND_PAGTO
...
--Criação da Tabela PEDIDO COMPRAS
...
--RH
--~Criação Tabela CENTRO DE CUSTO
...
--Financeiro
--Criação da Tabela CONTAS A RECEBER
...
--COMERCIAL
--Criação da Tabela PEDIDO DE VENDAS
...
--Criação de Tabela GERENTE DE VENDAS
...
--Criação da Tabela para registrar META DE VENDAS MÊS A MÊS/ANO
...
--FISCAL
--Criação da Tabela dos CÓDIGOS DE OPERAÇÕES FISCAIS
--DROP TABLE CFOP
CREATE TABLE CFOP(
...
--Criação da Tabela NOTA_FISCAL_ITENS
...
--Criação de Tabela PARÂMETRO DE INSS
...
--Criação de Tabela de PARÂMETRO DO IRRF
...
--Criação de Tabela AUDIT SALÁRIO
...
--Tabela de Parâmetros para autonumeração
...
--ADD CAMPO LOGIN TABELA APONTAMENTOS CRIAÇÃO APÓS TABELA USUÁRIOS E FK
...
--ADD CAMPO LOGIN TABELA ESTOQUE_MOV CRIAÇÃO APÓS TABELA USUÁRIO
ALTER TABLE ESTOQUE_MOV ADD LOGIN VARCHAR(30) NOT NULL

--NÃO APLICADOS EXEMPLOS
...
--ALTERAR COD_PAGTO TAB PED_COMPRAS PARA FOREIGN KEY APÓS TABELA COND_PAGTO
